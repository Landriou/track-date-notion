...
startProcess().then(() => {
  setInterval(RepeatProcess, 100000);
});
...
async function getTasksAndUpdateFromNotionDatabase() {
            ...
            and: [
              {
                property: "Status",
                status: {
                  equals: "Empatizacion / Research UX",
                },
              },
              {
                property: "DateStarted",
            ...
            and: [
              {
                property: "Status",
                status: {
                  equals: "Condensador de flujos / Definir",
                },
              },
              {
                property: "DateStarted",
  ...
  for (const page of pages) {
    ...
    const dateStartedPropertyId = page.properties["DateStarted"].id;
    const dateStartedPropertyItem = await getPropertyValue({
      pageId,
      propertyId: dateFinishedPropertyId,
    });
    const dateStarted = dateFinishedPropertyItem.date;
    ...
    if (dateFinished == null) {
      if (status == "Produccion" || status == "Done Design") {
    ...
    if (dateFinished == null) {
      if (
        ...
        status == "Empatizacion / Research UX" ||
        status == "Condensador de flujos / Definir" ||
        status == "Diseñar"
      ) {
        var tzoffset = new Date().getTimezoneOffset() * 60000; //offset in milliseconds
        var localISOTime = new Date(Date.now() - tzoffset)
